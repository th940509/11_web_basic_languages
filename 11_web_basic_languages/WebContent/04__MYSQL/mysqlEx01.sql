/* 
	...
	# SQL (Structured Query Language)
        ...
        - 데이터베이스 언어 SQL 문법의 종류는 다음 세 가지로 구별된다.
			...
			1) 데이터 정의 언어 (DDL : Data Definition Language)   Ex) Create , Drop , Alter    (*C:CREATE 만들기 / R:READ 읽기 / U:UPDATE 바꾸기 / D:DELETE 지우기)
...
# 예시 1번) 데이터 베이스 생성 (새로고침으로 확인해야 한다.)
CREATE DATABASE TEST_DB;

# 예시 2번) 데이터 베이스 사용 (*파일명 더블클릭해서 사용해도 됨)
USE TEST_DB;

# 예시 3번) 테이블 생성
CREATE TABLE MEMBER (
	NUM INT,             # 숫자 데이터 타입 : INT
    NAME VARCHAR(10),    # 글자 데이터 타입 : VARCHAR(최대길이)
                         #                문자열의 길이만큼만 데이터의 용량을 사용한다.
	HEIGHT DOUBLE,       # 실수 데이터 타입 : DOUBLE
    WEIGHT DOUBLE,
...
INSERT INTO MEMBER # 전체 컬럼의 데이터 추가이면 생략이 가능하다.
VALUES (
		2,
        '이영희',
        170.3,
...
INSERT INTO MEMBER( # 특정 컬럼만 데이트를 삽입할 경우, 특정 컬럼만 작성한다.
		NUM,
        NAME)
VALUES (
		3,
...
# 예시 7번) 데이터 수정 쿼리
# issue : mysql에서 update의 delete구문을 막아놓아서 safety update 옵션을 해지해야 update, delete문을 사용할 수 있다.
# edit > preperences > SQL editor > safety update 체크박스 해지
